...
function plotDecay( x, y, xdata, ydata, solutionPixel, solutionVectorChunks )

    % Init.
    pixel    = [ y, x ];
    measured = squeeze( ydata( pixel(1), pixel(2), : ));
    t        = 0:1.1*max(xdata);
    func     = @(t,param) param(1) * exp( - t ./ param(2)) + param(3);
    ...
    xlabel('echo time / ms');
    ylabel('signal / a.u.');
    legend('measurement', 'pixel-by-pixel fit', 'piecewise simultaneous fit');
    snapnow;
